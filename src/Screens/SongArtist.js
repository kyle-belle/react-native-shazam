...
const {song_artist_image_container, song_artist_name} = Styles;

const AnimatedFlatList = Animated.createAnimatedComponent(FlatList)

const SongArtist = ({song, loading_song, playing_song, route, position}) => {
    ...
    return (
        <>
        <AnimatedFlatList style={{minHeight: height, overflow: "visible"}} onScroll={Animated.event([{nativeEvent: {contentOffset: {y: scrollY}}}])} ListHeaderComponent={() => {
            return (
                <>
        ...
        }} data={songs} keyExtractor={(item, index) => index.toString()} renderItem={({item, index}) => <SongListItem playing={playing} loading={song_loading} index={index} />} contentContainerStyle={{paddingHorizontal: 20}} />
        
        <Animated.View style={{height: 95, width, position: "absolute", backgroundColor, transform: [{translateX}], opacity}} />
        </>
    );
}
